...
export async function postJobOneOffItemHandler(request: CallableRequest) {
	try {
		...
		return await postJobOneOffItem(simproJobId, description, cost);
	} catch (error: any) {
		return handleAxiosError(error);
	}
}

export async function postJobOneOffItem(
	simproJobId: string,
	description: string,
	cost: number
) {
	...
	if (sections.length == 0) {
		...
		const postCostCentreResponse: AxiosResponse = await simproApiService.post(
			postUetMaintenanceCostCentre(simproJobId, sectionId),
